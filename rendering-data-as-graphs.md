...
到目前为止都很顺利，不过这样对人类不是十分友好。一个可视化图可以很好地帮助我们了解这些语言计数的分布情况。我们把计数值传给 D3 来获得一个整洁的条形图，显示我们使用的编程语言的流行程度。
...
D3 是一个 JavaScript 库，并且喜欢把数据以数组形式处理。所以，我们先把我们的 Ruby Hash 转换成一个 JSON 数组，以便在浏览器内被 JavaScript 使用。
...
这时，lang_freq.erb 将会需要一些 JavaScript 语句来渲染条形图，在本例中你可以直接使用下方提供的代码，如果你想了解 D3 如何工作，还能同时参照上方的资源链接：
...
坦白的时候到了：存储库内的 `language` 属性只能识别“主要”的编程语言，这意味着如果你有一个存储库使用了几种不同的编程语言，那么只有最大的代码文件所使用的编程语言算数。

我们来结合几种 API 调用来获得一个能显示哪种编程语言拥有最多字节的代码的*真实表示*。[treemap](http://bl.ocks.org/mbostock/4063582) 是可以很好地可视化编程语言占用大小的方法，而不是像上面那样简单的计数。为此我们需要构造类似这样子的一个对象数组：
...
当当当当！一个美丽的矩形包含着你的储存库内各种编程语言，面积均以相对比例显示，一看就懂。为了能适当的显示所有的信息，你可能还需要对你的 treemap 的高度和宽度进行一些调整，这两个参数就是上面的 `drawTreemap` 的参数中的前两个。